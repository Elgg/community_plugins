<div class="sidebarBox">
	<h3><?php echo elgg_echo("Documentation"); ?></h3>
	<div class="sidebarWrapper">
		<ul id="documentation_links">
			<li><a href="http://docs.elgg.org/wiki/Install_Troubleshooting">Install Troubleshooting</a></li>
			<li><a href="http://docs.elgg.org/wiki/Category:Administration_FAQ">Using Elgg FAQ</a></li>
			<li><a href="http://docs.elgg.org/wiki/Adding_a_FAQ">Adding a new FAQ item</a></li>
			<li><a href="http://docs.elgg.org/wiki/Development/IssueTracker">Submiting a bug report</a></li>
			<li>&nbsp;</li>
			<li><a href="http://docs.elgg.org/wiki/Category:Development_FAQ">Developers FAQ</a></li>
			<li><a href="http://docs.elgg.org/wiki/Development/Subversion">Subversion repository</a></li>
		</ul>
	</div>
</div>