        ...
        switch ($key) {
            ...
            case 'l' :
            	if (is_array($settings['licence']) && in_array('enabled', $settings['licence'])) {
                	...
                	if (is_array($value) && !empty($value)) {
                		...
                		$options['joins'][] = "INNER JOIN {$dbprefix}metadata lm ON (e.guid = lm.entity_guid)";
	                    $options['joins'][] = "INNER JOIN {$dbprefix}metastrings ls_name ON (lm.name_id = ls_name.id AND ls_name.string = 'license')";
	                    $options['joins'][] = "INNER JOIN {$dbprefix}metastrings ls_value ON (lm.value_id = ls_value.id AND ls_value.string IN $licences)";
                	}
            	}
                break;
            case 'v' :
            ...
            case 's' :
            	if (isset($settings['screenshot']) && $settings['screenshot'] == 'enabled') {
                	...
                	$options['joins'][] = "INNER JOIN {$dbprefix}entity_relationships r on (r.relationship=\"image\" AND r.guid_one = e.guid)";
            	}
            	break;
        }
    }
...
if ((isset($settings['sort']) && $settings['sort'] == 'enabled') || empty($settings)) {
    switch($sort) {
    	...
    	case 'updated':
    		...
    		$options['joins'][] = "INNER JOIN {$dbprefix}entities er ON (er.container_guid = e.guid)";
